...
//
// 型番の集め方（探索）：
//   このAPIには「型番の一覧を全部ちょうだい」という一括取得手段が無いため、
//   検索ボックスに文字を打ったときのオートコンプリート（候補表示）と同じ
//   エンドポイントを使い、"HDL" "APS" "APX" などの先頭文字列から始めて、
//   候補が多そうな場合（＝まだ絞り込み不足）は次の1文字を足してさらに
//   問い合わせる……という形で候補を掘り下げていく（幅優先探索）。
//   API側の1回あたりの返却件数の上限が正確にいくつかは公開されていないため、
//   安全側に倒して「RESULT_EXPAND_THRESHOLD件以上返ってきたら深掘りする」
//   という閾値判定にしている。想定より深く潜ってしまうのを防ぐため、
//   問い合わせ回数の上限（MAX_TOTAL_REQUESTS）も設けている。
//
// 【注意】このスクリプトは実行環境の制約上、作成時に実際のAPIへ
// 疎通確認ができていません。初回実行時は、コンソールに出る型番の
// 発見件数や総リクエスト数のログを確認し、極端に少ない／MAX_TOTAL_REQUESTS
// に張り付いている場合はご一報ください（探索ロジックの調整が必要です）。
...
// このAPIが1回の問い合わせで返す上限件数は不明なため、
// 「この件数以上返ってきたら、まだ絞り込み不足とみなして深掘りする」の閾値。
const RESULT_EXPAND_THRESHOLD = 15;
